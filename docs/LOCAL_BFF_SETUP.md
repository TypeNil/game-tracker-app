...
По умолчанию `ServerConfig` привязывает сокет к loopback **`127.0.0.1:8080`**. Сервис принимает соединения только с локального компьютера.

Сценарии эмулятора (`10.0.2.2`) и USB (`adb reverse`) работают с этой привязкой: оба маршрутизируют трафик устройства на `127.0.0.1` хоста.

### LAN — явное включение

Привязка ко всем интерфейсам (`0.0.0.0`) — opt-in. У BFF нет клиентской аутентификации: открытие всех интерфейсов выставляет IGDB-квоту в локальную сеть.

- **POSIX**:
  ```bash
  HOST=0.0.0.0 ./gradlew :backend:run
  ```
- **Windows PowerShell**:
  ```powershell
  $env:HOST = "0.0.0.0"
