...
5. **Исключение: лента For You.** Ранжированный список рекомендаций живёт в памяти loader/ViewModel, не в Room. Каталог, поиск, details и библиотека остаются Room SSOT. После смерти процесса `live` For You не восстанавливается из локального кэша.
...
- **Известное ограничение offset-пагинации**: filter-only сортировка по умолчанию использует уникальный `id` (полный порядок). Явные однополевые сортировки (`rating`, `first_release_date`, `name`) подвержены ничьям равных значений на стороне IGDB. Клиентский `distinctBy { id }` отбрасывает кросс-страничные дубликаты, но не заполняет дыры, если порядок ничьей перетасовывается между страницами. Отсортированные ленты нельзя считать строго полными или стабильными.
